...
Download Raspbian, unzip it, and then write the `img` file to your micro SD card. Instead of rolling the old school `dd` utilitily on the commandline, I really like the UI and simplicity of [Etcher](https://etcher.io/), which is easy, while safer; you're not going to overwrite your root partition with it!
...
If you're not on Linux or macOS, just save a blank file named `ssh` to the boot partition, and you'll be fine.

Put the SD card in your Raspberry Pi, mount the DAC card as described in the instructions, connect power, connect network, and finally hook up the RCA cables from the DAC to your stereo, then boot the Pi. Once it's up, find it on your network, if you have a monitor you could just plugin the HDMI cable and watch the output, but I always just hit my router and look for it in the active LAN client list.
...
Remove default user, add your own user that will use sudo for administration, and have access to the audio hardware. When I create my user I'll use `fak3r`, you can use whatever you want, just subsitute as you go.
...
Next, we'll lock down who can login over `ssh`, and restrict that to our local subnet. Edit `/etc/ssh/sshd_config`, and Uncomment the `AllowUsers` and `ListenAddress` line, you will add your user and local subnet to restrict the logins. Again, subsitute your user, restrict it by only allowing it in from your subnet, and your listen address on your private subnet.pt-get -y install curl apt-transport-https
